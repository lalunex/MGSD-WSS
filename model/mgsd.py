...
class MGSD(SequentialRecommender):

    def __init__(self, config, dataset):
        super(MGSD, self).__init__(config, dataset)
        ...
        # 下面是用于替换高斯核的mlp
        self.replace_gaussian_fc1 = nn.Linear(self.embedding_size, self.embedding_size, True)
        self.replace_gaussian_fc2 = nn.Linear(self.embedding_size, self.embedding_size, False)
    ...
    def sequence_encoder_with_gaussian_kernel(self, item_seq, item_seq_emb, target_item):
        ...
        # 下面的语句用来检查MLP与高斯核的替换性如何
        replace_gaussian_item_seq_emb = self.relu(self.replace_gaussian_fc1(item_seq_emb))
        temp_raw_item_seq_emb_sumed = self.relu(self.replace_gaussian_fc2(replace_gaussian_item_seq_emb))
    ...
    def forward(self, interaction, train_flag=True):
        ...
        return terminate_user_emb, final_noise_weight, final_hard_noise_flag, final_interest_weight, final_hard_interest_flag
    ...
    def calculate_reweight_loss(self, interaction, gumbel_tau, spu_cl_tau):
        ...
        seq_emb_output, final_noise_weight, final_hard_noise_flag, final_interest_weight, final_hard_interest_flag = self.forward(interaction, train_flag=True)
        ...
        recommender_ce_losses_list = self.cross_entropy_loss(interaction, seq_emb_output, True)
    ...
    def predict(self, interaction):
        seq_emb_output, _, _, _ = self.forward(interaction, train_flag=False)
        test_item = interaction[self.ITEM_ID]
        test_item_emb = self.item_embedding(test_item)
        scores = torch.mul(seq_emb_output, test_item_emb).sum(dim=1)  # [B]

        return scores

    # 下面函数与recbole里面的代码很不一样
    def full_sort_predict(self, interaction):
        seq_emb_output, _, _, _, _ = self.forward(interaction, train_flag=False)

        test_items_emb = self.item_embedding.weight[:self.n_items]  # delete masked token
        scores = torch.matmul(seq_emb_output, test_items_emb.transpose(0, 1))  # [B, item_num]
