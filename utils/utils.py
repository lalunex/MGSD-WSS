...
LastEditTime: 2024-10-18 18:44:57
FilePath: /add_noise/utils/utils.py
...
def is_mgsd_model(model_name):
    return 'MGSD' in model_name
...
def indicators_20(predict, ground_true, seq_length):
    ...
    # ml-100k, 序列长度为18-50，所以分为0-49， 50
    # beauty, 序列长度为4-50，所以分为4-5， 6-50
    # sports，所以分为4-5， 6-50
    # yelp, 所以分为4， 5-7， 8-50
    # ml-1m, 所以分为0-54， 55-167， 167-200
    short_predict_seq = predict[np.where(seq_length <= 28)]
    short_ground_true = np.expand_dims(ground_true[np.where(seq_length <= 28)], axis=-1)
    ...
    medium_predict_seq = predict[np.where((seq_length >= 29) &  (seq_length <= 49))]
    medium_ground_true = np.expand_dims(ground_true[np.where((seq_length >= 29) & (seq_length <= 49))], axis=-1)
    ...
    long_predict_seq = predict[np.where(seq_length >= 50)]
    long_ground_true = np.expand_dims(ground_true[np.where(seq_length >= 50)], axis=-1)
