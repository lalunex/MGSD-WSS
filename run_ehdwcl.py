...
from model.ehdwcl import EHDWCL

from Triainer.EhdwclTrainer import EhdwclTrainer
from utils.utils import get_model, is_ehdwcl_model
...
def main(
    ...
    config = Config(
        model=EHDWCL if is_ehdwcl_model(model) else model,
        dataset=dataset,
    ...
    trainer = EhdwclTrainer(config, model)
    ...
    # test_result = trainer.evaluate(test_data, model_file='saved/different_seq_length/sports/EHDWCL-Dec-01-2023_02-26-07.pth')

    logger.info('best valid result: {}'.format(best_valid_result))
    logger.info('test result: {}'.format(test_result))

    if 'EHDWCL' in config_dict['model_name']:
...
def get_parameter_dict(dic):
    ...
    if dic['model_name'] == 'BERT4Rec' or (is_ehdwcl_model(dic['model_name']) and dic['sub_model'] == 'BERT4Rec'):
        bert_dict = {
    ...
    elif dic['model_name'] == 'GRU4Rec' or (is_ehdwcl_model(dic['model_name']) and dic['sub_model'] == 'GRU4Rec'):
        gru_dict = {
            "embedding_size": dic['embedding_size'],
            "hidden_size": dic['embedding_size'],
            "num_layers": 1,
    ...
    elif dic['model_name'] == 'SASRec' or (is_ehdwcl_model(dic['model_name']) and dic['sub_model'] == 'SASRec'):
        sas_dict = {
            "n_layers": 2,
            "n_heads": 2,
            "hidden_size": dic['embedding_size'],
    ...
    elif dic['model_name'] == 'Caser' or (is_ehdwcl_model(dic['model_name']) and dic['sub_model'] == 'Caser'):
        caser_dict = {
            "embedding_size": dic['embedding_size'],
            "hidden_size": dic['embedding_size'],
            "dropout_prob": 0.4,
    ...
    elif dic['model_name'] == 'NARM' or (is_ehdwcl_model(dic['model_name']) and dic['sub_model'] == 'NARM'):
        narm_dict = {
            "embedding_size": dic['embedding_size'],
            "hidden_size": dic['embedding_size'],
            "n_layers": 1,
    ...
    elif dic['model_name'] == 'STAMP' or (is_ehdwcl_model(dic['model_name']) and dic['sub_model'] == 'STAMP'):
        stamp_dict = {
            "embedding_size": dic['embedding_size'],
            "hidden_size": dic['embedding_size'],
            # "loss_type": 'CE'
...
output_file_name = 'model_experiment_output_interest1.xlsx'
...
if __name__ == '__main__':
    ...
    # parameter_dict['gpu_id'] = 1
    # parameter_dict['train_set_ratio'] = .2
    ...
    for seed in seed_list:
        parameter_dict['seed'] = seed
        parameter_dict['gpu_id'] = 0
        parameter_dict['user_interest_num'] = 5
